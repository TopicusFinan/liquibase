...
/**
 * Core Liquibase facade. Although there are several ways of executing Liquibase
 * (Ant, command line, etc.) they are all wrappers around this class.
 */
public class Liquibase {
	...
	public Liquibase(String changeLogFile, ResourceAccessor resourceAccessor,
			DatabaseConnection conn) throws LiquibaseException {
		this(changeLogFile, resourceAccessor, DatabaseFactory.getInstance()
	...
	public Liquibase(String changeLogFile, ResourceAccessor resourceAccessor,
		...
		if (changeLogFile != null) {
			this.changeLogFile = changeLogFile.replace('\\', '/'); // convert to
																	// standard
																	// / if
																	// using
																	// absolute
	...
	/**
	 * Use this function to override the current date/time function used to
	 * insert dates into the database. Especially useful when using an
	...
	public void update(String contexts) throws LiquibaseException {
		...
		changeLogParameters
				.setContexts(StringUtils.splitAndTrim(contexts, ","));
		...
		try {
			...
			ChangeLogIterator changeLogIterator = getStandardChangelogIterator(
					contexts, changeLog);
	...
	public void visualize(String contexts) throws LiquibaseException {
		...
		try {
			...
			graphVizContentBuilder
					.append("digraph LiquibaseVisualChangelog {\n");

			changeLogIterator.run(new VisualizeVisitor(database,
					graphVizContentBuilder), database);
	...
	public void visualize(String contexts, Writer output)
		...
		Executor oldTemplate = ExecutorService.getInstance().getExecutor(
				database);
		...
		try {
			...
			if (output != null) {
				output.write(graphVizContentBuilder.toString());
				output.flush();
			}
	...
	protected ChangeLogIterator getStandardChangelogIterator(String contexts,
		...
		return new ChangeLogIterator(changeLog, new ShouldRunChangeSetFilter(
				database, ignoreClasspathPrefix), new ContextChangeSetFilter(
	...
	public void update(String contexts, Writer output)
		...
		Executor oldTemplate = ExecutorService.getInstance().getExecutor(
				database);
		LoggingExecutor loggingExecutor = new LoggingExecutor(ExecutorService
				.getInstance().getExecutor(database), output, database);
		...
		try {
			...
			ChangeLogIterator logIterator = new ChangeLogIterator(changeLog,
					new ShouldRunChangeSetFilter(database,
							ignoreClasspathPrefix), new ContextChangeSetFilter(
							contexts), new DbmsChangeSetFilter(database),
	...
	public void update(int changesToApply, String contexts, Writer output)
		...
		outputHeader("Update " + changesToApply
				+ " Change Sets Database Script");
	...
	private void outputHeader(String message) throws DatabaseException {
		...
		if (connection != null) {
			executor.comment("Against: " + connection.getConnectionUserName()
					+ "@" + connection.getURL());
		}
		executor.comment("Liquibase version: "
	...
	public void rollback(int changesToRollback, String contexts, Writer output)
		...
		Executor oldTemplate = ExecutorService.getInstance().getExecutor(
				database);
		ExecutorService.getInstance().setExecutor(
				database,
				new LoggingExecutor(ExecutorService.getInstance().getExecutor(
	...
	public void rollback(int changesToRollback, String contexts)
		...
		try {
			...
			ChangeLogIterator logIterator = new ChangeLogIterator(
					database.getRanChangeSetList(), changeLog,
					new AlreadyRanChangeSetFilter(database
							.getRanChangeSetList()),
	...
	public void rollback(String tagToRollBackTo, String contexts)
		...
		try {
			...
			List<RanChangeSet> ranChangeSetList = database
					.getRanChangeSetList();
			ChangeLogIterator logIterator = new ChangeLogIterator(
					ranChangeSetList, changeLog, new AfterTagChangeSetFilter(
							tagToRollBackTo, ranChangeSetList),
	...
	public void rollback(Date dateToRollBackTo, String contexts)
		...
		try {
			...
			List<RanChangeSet> ranChangeSetList = database
					.getRanChangeSetList();
			ChangeLogIterator logIterator = new ChangeLogIterator(
					ranChangeSetList, changeLog,
					new ExecutedAfterChangeSetFilter(dateToRollBackTo,
							ranChangeSetList), new AlreadyRanChangeSetFilter(
							ranChangeSetList), new ContextChangeSetFilter(
							contexts), new DbmsChangeSetFilter(database));
	...
	public void changeLogSync(String contexts, Writer output)
		...
		LoggingExecutor outputTemplate = new LoggingExecutor(ExecutorService
				.getInstance().getExecutor(database), output, database);
		Executor oldTemplate = ExecutorService.getInstance().getExecutor(
				database);
	...
	public void futureRollbackSQL(Integer count, String contexts, Writer output)
			throws LiquibaseException {
		contexts = StringUtils.trimToNull(contexts);
		changeLogParameters
				.setContexts(StringUtils.splitAndTrim(contexts, ","));
		...
		try {
			...
			if (count == null) {
				logIterator = new ChangeLogIterator(changeLog,
						new NotRanChangeSetFilter(database
								.getRanChangeSetList()),
						new ContextChangeSetFilter(contexts),
						new DbmsChangeSetFilter(database));
			} else {
				ChangeLogIterator forwardIterator = new ChangeLogIterator(
						changeLog, new NotRanChangeSetFilter(
	...
	public void checkDatabaseChangeLogTable(
		...
		if (updateExistingNullChecksums && databaseChangeLog == null) {
			throw new LiquibaseException(
					"changeLog parameter is required if updating existing checksums");
		}
		String[] splitContexts = null;
		if (StringUtils.trimToNull(contexts) != null) {
			splitContexts = contexts.split(",");
		}
		getDatabase().checkDatabaseChangeLogTable(updateExistingNullChecksums,
				databaseChangeLog, splitContexts);
	...
	/**
	 * Returns true if it is "save" to migrate the database. Currently, "safe"
	 * is defined as running in an output-sql mode or against a database on
	 * localhost. It is fine to run Liquibase against a "non-safe" database, the
	 * method is mainly used to determine if the user should be prompted before
	 * continuing.
	 */
	...
	public void reportLocks(PrintStream out) throws LiquibaseException {
		...
		out.println("Database change log locks for "
				+ getDatabase().getConnection().getConnectionUserName() + "@"
	...
	public List<ChangeSet> listUnrunChangeSets(String contexts)
		...
		ChangeLogIterator logIterator = getStandardChangelogIterator(contexts,
				changeLog);
	...
	public void reportStatus(boolean verbose, String contexts, Writer out)
			throws LiquibaseException {
		contexts = StringUtils.trimToNull(contexts);
		changeLogParameters
				.setContexts(StringUtils.splitAndTrim(contexts, ","));
		...
		try {
			...
			if (unrunChangeSets.size() == 0) {
				out.append(getDatabase().getConnection()
	...
	public Collection<RanChangeSet> listUnexpectedChangeSets(String contexts)
		...
		ChangeLogIterator logIterator = new ChangeLogIterator(changeLog,
				new ContextChangeSetFilter(contexts), new DbmsChangeSetFilter(
						database));
		ExpectedChangesVisitor visitor = new ExpectedChangesVisitor(
				database.getRanChangeSetList());
		logIterator.run(visitor, database);
		return visitor.getUnexpectedChangeSets();
	}

	public void reportUnexpectedChangeSets(boolean verbose, String contexts,
			...
			if (unexpectedChangeSets.size() == 0) {
				...
				if (verbose) {
					for (RanChangeSet ranChangeSet : unexpectedChangeSets) {
						out.append("     ").append(ranChangeSet.toString())
								.append(StreamUtil.getLineSeparator());
					}
				}
			}
	...
	public void clearCheckSums() throws LiquibaseException {
		...
		try {
			...
			UpdateStatement updateStatement = new UpdateStatement(getDatabase()
					.getLiquibaseCatalogName(), getDatabase()
					.getLiquibaseSchemaName(), getDatabase()
	...
	public final CheckSum calculateCheckSum(final String changeSetIdentifier)
			throws LiquibaseException {
		if (changeSetIdentifier == null) {
			throw new LiquibaseException(new IllegalArgumentException(
					"changeSetIdentifier"));
		}
		final List<String> parts = StringUtils.splitAndTrim(
				changeSetIdentifier, "::");
		if (parts == null || parts.size() < 3) {
			throw new LiquibaseException(new IllegalArgumentException(
					"Invalid changeSet identifier: " + changeSetIdentifier));
		}
		return this.calculateCheckSum(parts.get(0), parts.get(1), parts.get(2));
	}

	public CheckSum calculateCheckSum(final String filename, final String id,
			final String author) throws LiquibaseException {
		log.info(String.format("Calculating checksum for changeset %s::%s::%s",
				filename, id, author));
		final ChangeLogParameters changeLogParameters = this
				.getChangeLogParameters();
		final ResourceAccessor resourceAccessor = this.getFileOpener();
		final DatabaseChangeLog changeLog = ChangeLogParserFactory
				.getInstance()
				.getParser(this.changeLogFile, resourceAccessor)
				.parse(this.changeLogFile, changeLogParameters,
						resourceAccessor);

		// TODO: validate?

		final ChangeSet changeSet = changeLog
				.getChangeSet(filename, author, id);
		if (changeSet == null) {
			throw new LiquibaseException(new IllegalArgumentException(
					"No such changeSet: " + filename + "::" + id + "::"
							+ author));
		}
	...
	public void generateDocumentation(String outputDirectory, String contexts)
			throws LiquibaseException {
		contexts = StringUtils.trimToNull(contexts);
		...
		// }
		//
		// DBDocChangeLogHandler changeLogHandler = new
		// DBDocChangeLogHandler(outputDirectory, this,
	...
	public DiffResult diff(Database referenceDatabase, Database targetDatabase,
			CompareControl compareControl) throws LiquibaseException {
		return DiffGeneratorFactory.getInstance().compare(referenceDatabase,
				targetDatabase, compareControl);
	}

	/**
	 * Checks changelogs for bad MD5Sums and preconditions before attempting a
	 * migration
	 */
	 ...
	 * Safe properties are the ones that doesn't have side effects in liquibase
	 * state and also don't change in during the liquibase execution
	 * 
	 * @param database
	 *            Database which propeties are put in the changelog
	 * @throws DatabaseException
	 */
	private void setDatabasePropertiesAsChangelogParameters(Database database)
			throws DatabaseException {
		setChangeLogParameter("database.autoIncrementClause",
				database.getAutoIncrementClause(null, null));
		...
		setChangeLogParameter("database.defaultCatalogName",
				database.getDefaultCatalogName());
		setChangeLogParameter("database.defaultSchemaName",
				database.getDefaultSchemaName());
		setChangeLogParameter(
				"database.defaultSchemaNamePrefix",
		...
		setChangeLogParameter("database.requiresPassword",
				database.requiresPassword());
		setChangeLogParameter("database.requiresUsername",
		...
		setChangeLogParameter("database.supportsSchemas",
				database.supportsSchemas());
		setChangeLogParameter("database.supportsSequences",
				database.supportsSequences());
		setChangeLogParameter("database.supportsTablespaces",
